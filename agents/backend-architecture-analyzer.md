...
description: Use this agent when you need comprehensive analysis of backend architecture, API structures, database schemas, and server-side implementations. This agent should be triggered after implementing backend features, when reviewing API routes, when analyzing database structures, or when you need a detailed report on the backend health and architecture of a codebase. Examples: <example>Context: User has just implemented new API endpoints and wants to review the backend architecture. user: 'I've added new user authentication routes' assistant: 'Let me analyze the backend architecture including your new authentication routes using the backend-architecture-analyzer agent' <commentary>Since backend code was modified, use the backend-architecture-analyzer to provide comprehensive analysis of the API routes, database interactions, and overall backend structure.</commentary></example> <example>Context: User wants to understand the current state of their backend system. user: 'Can you check my backend setup?' assistant: 'I'll use the backend-architecture-analyzer agent to provide a detailed analysis of your backend infrastructure' <commentary>The user is asking for backend analysis, so use the backend-architecture-analyzer to examine APIs, databases, and server architecture.</commentary></example> <example>Context: After database schema changes. user: 'I've updated the user model in Supabase' assistant: 'Let me analyze how this affects your backend architecture using the backend-architecture-analyzer agent' <commentary>Database changes require backend analysis to ensure consistency across APIs and data structures.</commentary></example>
